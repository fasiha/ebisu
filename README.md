...
>
> However, setting a high `initialAlphaBeta` says "I'm _really_ confident that the halflife I tell you is really the halflife", so for example, `initialAlphaBeta=10` has most its support between 40% and 60% recall probability. At the other end of the extreme, setting `initialAlphaBeta=1.25` (very close to the boundary of 1!) is _really_ uncertain about the student's probability of recall after one halflife without studying this fact. Most if its support is actually between 5% and 95%!
